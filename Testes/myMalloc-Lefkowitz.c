...
ListaRegistroChaveSecundaria *cria_lista_registro_chave_segundaria(ListaClientes *lc)
{
	ListaRegistroChaveSecundaria *r = (ListaRegistroChaveSecundaria *)MyMalloc(sizeof(ListaRegistroChaveSecundaria));

	r->lista = (RegistroChaveSecundaria**)MyMalloc(sizeof(RegistroChaveSecundaria*) * lc->qtd);
	r->qtd = lc->qtd;

	int i;
	for (i = 0; i < lc->qtd; i++) {
		r->lista[i] = (RegistroChaveSecundaria *)MyMalloc(sizeof(RegistroChaveSecundaria));
...
Cliente *cliente(int cod, char *nome, int idade)
{
	Cliente *cliente = (Cliente *) MyMalloc(sizeof(Cliente));
...
{
	va_list ap;
	ListaClientes *lc = (ListaClientes *)  MyMalloc(sizeof(ListaClientes));
	lc->qtd = qtd;
	lc->lista = (Cliente **) MyMalloc(sizeof(Cliente *) * (qtd));
	int i;
	va_start(ap, qtd);
	for (i = 0; i < qtd; i++) {
...
void libera_clientes(ListaClientes *lc)
{
	int i;
	for (i = 0; i < lc->qtd; i++) {
		MyMallocFree(lc->lista[i]);
	}
	MyMallocFree(lc->lista);
	MyMallocFree(lc);
}
...
RegistroIndiceIdade *registro_indice_idade(int idade, int pt, int q)
{
	RegistroIndiceIdade *registro_indice_idade = (RegistroIndiceIdade *) MyMalloc(sizeof(RegistroIndiceIdade));
...
RegistroIndiceIdade *le_registro_indice_idade(FILE *in)
{
	...
	if (0 >= fread(&registro_indice_idade->idade, sizeof(int), 1, in)) {
		MyMallocFree(registro_indice_idade);
		return NULL;
	}
...
ListaRegistrosIndiceIdade *cria_registros(ListaRegistroChaveSecundaria *lcs)
{
	ListaRegistrosIndiceIdade *lc = (ListaRegistrosIndiceIdade *)  MyMalloc(sizeof(ListaRegistrosIndiceIdade));
	lc->lista = (RegistroIndiceIdade **) MyMalloc(sizeof(RegistroIndiceIdade *) * (lcs->qtd));
	...
	for (i = 0; i < lcs->qtd; i++) {
		...
		{
			j++;
			lc->lista[j] = (RegistroIndiceIdade*)MyMalloc(sizeof(RegistroIndiceIdade));
...
{
	int qtd = 0;
	ListaRegistrosIndiceIdade *l = (ListaRegistrosIndiceIdade *)  MyMalloc(sizeof(ListaRegistrosIndiceIdade));
	FILE *in = fopen(nome_arquivo, "rb");
	if (in != NULL) {
		RegistroIndiceIdade *registro = NULL;
		while((registro = le_registro_indice_idade(in)) != NULL) {
			qtd += 1;
			MyMallocFree(registro);
		}
		fseek(in, 0, SEEK_SET);
		l->qtd = qtd;
		l->lista = (RegistroIndiceIdade **) MyMalloc(sizeof(RegistroIndiceIdade *) * (qtd));
		qtd = 0;
		while((registro = le_registro_indice_idade(in)) != NULL) {
...
void libera_registros(ListaRegistrosIndiceIdade *l)
{
	int i;
	for (i = 0; i < l->qtd; i++) {
		MyMallocFree(l->lista[i]);
	}
	MyMallocFree(l->lista);
	MyMallocFree(l);
}
...
ListaRegistroChavePonteiro *cria_lista_registro_chave_ponteiro(ListaRegistroChaveSecundaria *lc)
{
	ListaRegistroChavePonteiro *lcp = (ListaRegistroChavePonteiro *)MyMalloc(sizeof(ListaRegistroChavePonteiro));

	lcp->lista = (RegistroChavePonteiro**)MyMalloc(sizeof(RegistroChavePonteiro*) * lc->qtd);
	lcp->qtd = lc->qtd;
	int i;
	for(i = 0; i < lc->qtd; i++)
	{
		lcp->lista[i] = (RegistroChavePonteiro*)MyMalloc(sizeof(RegistroChavePonteiro));
...
ListaFinal* criar_lista_final(ListaClientes *lc, ListaRegistroChavePonteiro *lcp)
{
	ListaFinal *lf = (ListaFinal*)MyMalloc(sizeof(ListaFinal));

	lf->lista = (RegistroFinal**)MyMalloc(sizeof(RegistroFinal*) * lc->qtd);
	lf->qtd = lc->qtd;

	int i;
	for(i = 0; i < lc->qtd; i++)
	{
		lf->lista[i] = (RegistroFinal*)MyMalloc(sizeof(RegistroFinal));
